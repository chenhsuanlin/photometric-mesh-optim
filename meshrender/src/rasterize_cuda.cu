...
__device__ void vector_sub_cuda(float* res,float* a,float* b){ for (int i = 0; i<3; i++) res[i] = a[i]-b[i]; }
__device__ float dot_product_cuda(float* a,float* b){ return a[X]*b[X]+a[Y]*b[Y]+a[Z]*b[Z]; }
...
__global__
void forward_cuda_kernel(int num_faces,int H,int W,float *CI,float *FVT,int *BFI,int *IM,float *BM,float *IDM,int *LM)
{
...
int forward_cuda_launcher(int num_faces,int H,int W,float *CI,float *FVT,int *BFI,int *IM,float *BM,float *IDM,int *LM)
{
    const int T = 256;
    forward_cuda_kernel<<<(num_faces+T-1)/T,T>>>(num_faces,H,W,CI,FVT,BFI,IM,BM,IDM,LM);
    return 1;
}
